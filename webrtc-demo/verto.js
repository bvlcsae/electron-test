...
function check_vid_res()
{
    ...
    } else if ($("#vqual_qvga2").is(':checked')) {
	vid_width = 320;
	vid_height = 320;
	local_vid_width = 160;
	local_vid_height = 120;
	...
	local_vid_width = 160;
	local_vid_height = 120;
    } else if ($("#vqual_vga2").is(':checked')) {
	vid_width = 640;
	vid_height = 640;
	local_vid_width = 160;
	local_vid_height = 120;
    } else if ($("#vqual_vga3").is(':checked')) {
	vid_width = 480;
	vid_height = 480;
	local_vid_width = 160;
	local_vid_height = 120;
    ...
    if ($("#minFrameRate_15").is(':checked')) {
	minFrameRate = 15;
    } else if ($("#minFrameRate_20").is(':checked')) {
	minFrameRate = 20;
    } else if ($("#minFrameRate_25").is(':checked')) {
	minFrameRate = 25;
	} else if ($("#minFrameRate_30").is(':checked')) {
	minFrameRate = 30;
	}

    if ($("#restFrameRate_15").is(':checked')) {
	bestFrameRate = 15;
    } else if ($("#bestFrameRate_20").is(':checked')) {
	bestFrameRate = 20;
    } else if ($("#bestFrameRate_25").is(':checked')) {
	bestFrameRate = 25;
	} else if ($("#bestFrameRate_30").is(':checked')) {
	bestFrameRate = 30;
	}
    real_size();
	...
	vertoHandle.videoParams({
	    ...
	    "vertoBestFrameRate": bestFrameRate,
...
function refresh_devices()
{
    ...
    var tmp;
    tmp = $.cookie("hente_demo_camera_selected") || "false";
    if (tmp) {
        $('#usecamera option[value=' + tmp + ']').prop('selected', 'selected').change();
        pop_select("#usecamera","hente_demo_camera_selected", tmp);
    }

    var tmp;
    tmp = $.cookie("hente_demo_share_selected") || "false";
    if (tmp) {
        $('#useshare option[value=' + tmp + ']').prop('selected', 'selected').change();
        pop_select("#useshare","hente_demo_share_selected", tmp);
    }

    tmp = $.cookie("hente_demo_mic_selected") || "false";
    if (tmp) {
        $('#usemic option[value=' + tmp + ']').prop('selected', 'selected').change();
        pop_select("#usemic","hente_demo_mic_selected", tmp);
    }

    tmp = $.cookie("hente_demo_speak_selected") || "false";
    if (tmp) {
        $('#usespeak option[value=' + tmp + ']').prop('selected', 'selected').change();
        pop_select("#usespeak","hente_demo_speak_selected", tmp);
    }
}
...
function init() {
    ...
    if (!autocall) {
	pop("#ext", "hente_demo_ext", "*8500");
    }

    pop("#avatar", "hente_demo_avatar", "", function(jq) {
	$("#avatar_img").attr("src", jq.val());
    });
    pop("#cidname", "hente_demo_name", "Hente User");
    pop("#cid", "hente_demo_cid", "1008");
    pop("#email", "hente_demo_emailaddr", "", function(jq) {
	$("#avatar").val("http://gravatar.com/avatar/" + md5($("#email").val()) + ".png?s=600");
	$("#avatar_img").attr("src", $("#avatar").val());
	$("#avatar").change();
    });
    pop("#textto", "hente_demo_textto", "1000");

    pop("#login", "hente_demo_login", "1");
    pop("#passwd", "hente_demo_passwd", "1");

    pop("#hostName", "hente_demo_hostname", window.location.hostname);
    pop("#wsURL", "hente_demo_wsurl", "wss://dxims.hente.cn:843");


    $("#avatar_img").attr("src", $("#avatar").val());

    var tmp = $.cookie("hente_demo_vid_checked") || "true";
    $.cookie("hente_demo_vid_checked", tmp, {
        expires: 365
    });
	...
	if (!tmp) {
        ...
        $.cookie("hente_demo_vid_checked", tmp ? "true" : "false", {
            expires: 365
        });
    });


    tmp = $.cookie("hente_demo_dedenc_checked") || "false";
    $.cookie("hente_demo_dedenc_checked", tmp, {
        expires: 365
    });
	...
	if (!tmp && $("#mirror_input").is(':checked')) {
        ...
        $.cookie("hente_demo_dedenc_checked", tmp ? "true" : "false", {
            expires: 365
        });
    });

    tmp = $.cookie("hente_demo_mirror_input_checked") || "false";
    $.cookie("hente_demo_mirror_input_checked", tmp, {
        expires: 365
    });
	...
	if (tmp && !$("#use_dedenc").is(':checked')) {
        ...
        $.cookie("hente_demo_mirror_input_checked", tmp ? "true" : "false", {
            expires: 365
        });
    });

//
    outgoingBandwidth = $.cookie("hente_demo_outgoingBandwidth") || "default";
    $.cookie("hente_demo_outgoingBandwidth", outgoingBandwidth, {
        expires: 365
    });
...
//
    ...
    incomingBandwidth = $.cookie("hente_demo_incomingBandwidth") || "default";
    $.cookie("hente_demo_incomingBandwidth", incomingBandwidth, {
        expires: 365
    });
...
//
    ...
    vqual = $.cookie("hente_demo_vqual") || "hd";
    $.cookie("hente_demo_vqual", vqual, {
        expires: 365
    });
    ...
    $("#vqual_qvga2").prop("checked", vqual === "qvga2").change(function(e) {
        if ($("#vqual_qvga2").is(':checked')) {
	    vqual = "qvga2";
	    $.cookie("hente_demo_vqual", vqual, {
		expires: 365
	    });
    ...
    $("#vqual_vga2").prop("checked", vqual === "vga2").change(function(e) {
        if ($("#vqual_vga2").is(':checked')) {
	    vqual = "vga2";
	    $.cookie("hente_demo_vqual", vqual, {
		expires: 365
	    });
    ...
    $("#vqual_vga3").prop("checked", vqual === "vga3").change(function(e) {
        if ($("#vqual_vga3").is(':checked')) {
	    vqual = "vga3";
	    $.cookie("hente_demo_vqual", vqual, {
		expires: 365
	    });
    ...
    minFrameRate = $.cookie("hente_demo_minFrameRate") || "25";
    $.cookie("hente_demo_minFrameRate", minFrameRate, {
        expires: 365
    });

    $("#minFrameRate_15").prop("checked", minFrameRate === "15").change(function(e) {
        if ($("#minFrameRate_15").is(':checked')) {
	    minFrameRate = 15;
	    $.cookie("hente_demo_minFrameRate", minFrameRate, {
		expires: 365
    ...
    $("#minFrameRate_20").prop("checked", minFrameRate === "20").change(function(e) {
        if ($("#minFrameRate_20").is(':checked')) {
	    minFrameRate = 20;
	    $.cookie("hente_demo_minFrameRate", minFrameRate, {
		expires: 365
	    });
	...
	$("#minFrameRate_25").prop("checked", minFrameRate === "25").change(function(e) {
        if ($("#minFrameRate_25").is(':checked')) {
	    minFrameRate = 25;
	    $.cookie("hente_demo_minFrameRate", minFrameRate, {
		expires: 365
	    });
	...
	$("#minFrameRate_30").prop("checked", minFrameRate === "30").change(function(e) {
        if ($("#minFrameRate_30").is(':checked')) {
	    minFrameRate = 30;
	    $.cookie("hente_demo_minFrameRate", minFrameRate, {
		expires: 365
	    });
    ...
    bestFrameRate = $.cookie("hente_demo_bestFrameRate") || "25";
    $.cookie("hente_demo_bestFrameRate", bestFrameRate, {
        expires: 365
    });

    $("#bestFrameRate_15").prop("checked", bestFrameRate === "15").change(function(e) {
        if ($("#bestFrameRate_15").is(':checked')) {
	    bestFrameRate = 15;
	    $.cookie("hente_demo_bestFrameRate", bestFrameRate, {
		expires: 365
	    });
    ...
    $("#bestFrameRate_20").prop("checked", bestFrameRate === "20").change(function(e) {
        if ($("#bestFrameRate_20").is(':checked')) {
	    bestFrameRate = 20;
	    $.cookie("hente_demo_bestFrameRate", bestFrameRate, {
		expires: 365
	    });
	...
	$("#bestFrameRate_25").prop("checked", bestFrameRate === "25").change(function(e) {
        if ($("#bestFrameRate_25").is(':checked')) {
	    bestFrameRate = 25;
	    $.cookie("hente_demo_bestFrameRate", bestFrameRate, {
		expires: 365
	    });
	...
	$("#bestFrameRate_30").prop("checked", bestFrameRate === "39").change(function(e) {
        if ($("#bestFrameRate_30").is(':checked')) {
	    bestFrameRate = 30;
	    $.cookie("hente_demo_bestFrameRate", bestFrameRate, {
		expires: 365
	    });
    ...
    tmp = $.cookie("hente_demo_stereo_checked") || "true";
    $.cookie("hente_demo_stereo_checked", tmp, {
        expires: 365
    });

    $("#use_stereo").prop("checked", tmp === "true").change(function(e) {
        tmp = $("#use_stereo").is(':checked');
        $.cookie("hente_demo_stereo_checked", tmp ? "true" : "false", {
            expires: 365
        });
    });

    tmp = $.cookie("hente_demo_stun_checked") || "true";
    $.cookie("hente_demo_stun_checked", tmp, {
        expires: 365
    });

    $("#use_stun").prop("checked", tmp === "true").change(function(e) {
        tmp = $("#use_stun").is(':checked');
        $.cookie("hente_demo_stun_checked", tmp ? "true" : "false", {
            expires: 365
        });
	if (vertoHandle) {
    ...
    tmp = $.cookie("hente_demo_local_video_checked") || "false";
    $.cookie("hente_demo_local_video_checked", tmp, {
        expires: 365
    });

    $("#local_video").prop("checked", tmp === "true").change(function(e) {
        tmp = $("#local_video").is(':checked');
        $.cookie("hente_demo_local_video_checked", tmp ? "true" : "false", {
            expires: 365
        });
    });
	...
	iceServers: false//$("#use_stun").is(':checked')
    },callbacks);
    ...
    $("#login").change(function(e) {
        ...
        $.cookie("hente_demo_cid", e.currentTarget.value, {
            expires: 365
        });
    });
